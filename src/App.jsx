...
function sb(table){
  ...
  const self={
    ...
    get(){
      let u=SB_URL+"/rest/v1/"+table+"?select="+_s;
      if(_f.length)u+="&"+_f.join("&");
      if(_o)u+="&"+_o;
      return fetch(u,{headers:HDR}).then(r=>r.json().then(d=>r.ok?{data:d,error:null}:{data:null,error:d}));
    },
...
function useDebourse(){
  ...
  const load=useCallback(()=>{
    ...
    Promise.all([sb("debourse_sessions").order("created_at",{ascending:false}).get(),sb("debourse_taches").order("ordre").get()])
      .then(([rs,rt])=>{setSessions(rs.data||[]);setTaches(rt.data||[]);setLoading(false);})
      .catch(()=>setLoading(false));
  },[]);
  useEffect(()=>{load();},[]);
...
function Spin(){return <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:70,gap:8}}><div style={{width:26,height:26,border:"3px solid #57534E",borderTopColor:"#F97316",borderRadius:"50%",animation:"spin 1s linear infinite"}}/><style>{"@keyframes spin{to{transform:rotate(360deg)}}"}</style></div>;}
    ...
    <div style={{display:"flex",height:"100vh",background:T.bg,color:T.white,fontFamily:T.fontFamily,overflow:"hidden"}}>
      ...
      <div style={{flex:1,overflow:"auto",display:"flex",flexDirection:"column",paddingBottom:isMobile?64:0}}>
        ...
        <div style={{flex:1,overflow:"auto",padding:isMobile?"10px":"20px"}}>
          {error?<ErrorState error={error} reload={reloadAll} T={T}/>:<>
            {page==="dashboard"&&<Dashboard ch={ch} intv={intv} openCh={openCh} T={T} isMobile={isMobile}/>}
            ...
            {page==="ia"&&<IA ch={ch} intv={intv} T={T}/>}
          </>}
        </div>
      </div>
...
function ErrorState({error,reload,T}){return <div style={{background:T.danger+"11",border:"1px solid "+T.danger+"44",borderRadius:12,padding:20,textAlign:"center"}}><div style={{color:T.danger,fontWeight:700,marginBottom:8}}>Erreur connexion</div><div style={{color:T.muted,fontSize:12,marginBottom:12}}>{error}</div><button onClick={reload} style={{background:T.primary,color:"#fff",border:"none",borderRadius:8,padding:"9px 20px",fontWeight:700,cursor:"pointer"}}>Réessayer</button></div>;}

// ── DASHBOARD ─────────────────────────────────────────────────────────────────
function Dashboard({ch,intv,openCh,T,isMobile}){
...
function Chantiers({ch,openCh,reload,T,isMobile}){
  ...
  function save(){if(!form.nom||!form.budget_initial)return;setSaving(true);sb("chantiers").insert({nom:form.nom,client:form.client,localisation:form.localisation,type:form.type,budget_initial:parseFloat(form.budget_initial),date_debut:form.date_debut||null,date_fin:form.date_fin||null,statut:"Brouillon"}).then(()=>{setSaving(false);setShowNew(false);reload();});}
  function del(id){if(!window.confirm("Supprimer ce chantier ?"))return;sb("chantiers").eq("id",id).del().then(()=>reload());}
...
// ── FICHE CHANTIER — dépenses multi-lignes ────────────────────────────────────
...
function Fiche({chantier:c,setPage,reload,T,isMobile}){
  ...
  async function saveDeps(){
    ...
    for(const l of valid){
      await sb("depenses").insert({chantier_id:c.id,libelle:l.libelle,categorie:l.categorie,montant:parseFloat(l.montant),date:l.date,note:l.note});
    }
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:0}}>
    ...
    <div style={{background:T.card,border:"1px solid "+T.border,borderRadius:T.borderRadius,padding:isMobile?14:18,marginBottom:12}}>
      <div style={{display:"flex",justifyContent:"space-between",gap:10,flexWrap:"wrap",marginBottom:10}}>
        <div style={{flex:1}}><div style={{fontSize:isMobile?16:20,fontWeight:800}}>{c.nom}</div><div style={{color:T.muted,fontSize:11,marginTop:3}}>{c.client} — {c.localisation}</div></div>
        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{STATUTS.map(st=><button key={st} onClick={()=>changeSt(st)} style={{padding:"4px 8px",borderRadius:20,border:"1px solid "+(c.statut===st?stC(st,T):T.border),background:c.statut===st?stC(st,T)+"22":"transparent",color:c.statut===st?stC(st,T):T.muted,cursor:"pointer",fontSize:10}}>{st}</button>)}</div>
      </div>
      <Badge label={c.statut} color={stC(c.statut,T)}/>
    </div>
    <div style={{display:"flex",gap:4,marginBottom:12,overflowX:"auto"}}>
      {[["infos","Infos"],["depenses","Dépenses ("+c.depenses.length+")"]].map(o=><button key={o[0]} onClick={()=>setTab(o[0])} style={{padding:"7px 12px",borderRadius:8,border:"1px solid "+(tab===o[0]?T.primary:T.border),background:tab===o[0]?T.primary:T.card,color:tab===o[0]?"#fff":T.muted,cursor:"pointer",fontSize:12,fontWeight:tab===o[0]?700:400}}>{o[1]}</button>)}
    </div>
    {tab==="infos"&&<div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:12}}>
      ...
      <Card title="Budget" T={T}>
        <div style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}}><span style={{color:T.muted}}>Avancement</span><strong style={{color:dp>100?T.danger:dp>80?T.warning:T.success}}>{dp}%</strong></div><PBar p={dp} color={dp>100?T.danger:dp>80?T.warning:T.success} h={12}/></div>
        {[["Budget initial",fmt(c.budgetInitial),T.white],["Dépenses",fmt(dep),T.warning],["Marge",fmt(c.budgetInitial-dep),c.budgetInitial-dep>=0?T.success:T.danger]].map(row=><div key={row[0]} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid "+T.border,fontSize:12}}><span style={{color:T.muted}}>{row[0]}</span><span style={{fontWeight:700,color:row[2]}}>{row[1]}</span></div>)}
      </Card>
    </div>}
    {tab==="depenses"&&<div style={{display:"flex",flexDirection:"column",gap:10}}>
      ...
      {/* Modal multi-lignes */}
      {showDep&&<Modal title={"Saisie de dépenses — "+c.nom} onClose={()=>setShowDep(false)} onSave={saveDeps} saveLabel={"Enregistrer "+lines.filter(l=>l.libelle&&l.montant).length+" ligne(s)"} T={T} wide>
        {saving?<Spin/>:<>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}>
              <thead><tr style={{background:T.mid}}>{["Libellé *","Catégorie","Montant (XOF) *","Date","Note",""].map((h,i)=><th key={i} style={{padding:"7px 8px",textAlign:"left",fontSize:10,color:T.muted,fontWeight:600}}>{h}</th>)}</tr></thead>
              <tbody>
                {lines.map(l=>{
                  ...
                  return <tr key={l.id} style={{borderBottom:"1px solid "+T.border+"44"}}>
                    <td style={{padding:"4px 4px"}}><input value={l.libelle} onChange={e=>upLine(l.id,"libelle",e.target.value)} placeholder="ex: Béton B25" style={iS}/></td>
                    <td style={{padding:"4px 4px"}}><select value={l.categorie} onChange={e=>upLine(l.id,"categorie",e.target.value)} style={iS}>{CATS.map(c=><option key={c} value={c}>{c}</option>)}</select></td>
                    <td style={{padding:"4px 4px"}}><input type="number" value={l.montant} onChange={e=>upLine(l.id,"montant",e.target.value)} placeholder="0" style={{...iS,width:110}}/></td>
                    <td style={{padding:"4px 4px"}}><input type="date" value={l.date} onChange={e=>upLine(l.id,"date",e.target.value)} style={{...iS,width:130}}/></td>
                    <td style={{padding:"4px 4px"}}><input value={l.note} onChange={e=>upLine(l.id,"note",e.target.value)} placeholder="Optionnel" style={iS}/></td>
                    <td style={{padding:"4px 4px"}}><button onClick={()=>removeLine(l.id)} style={{background:T.danger+"22",color:T.danger,border:"none",borderRadius:5,padding:"5px 8px",cursor:"pointer",fontSize:13}}>✕</button></td>
                  </tr>;
                })}
              </tbody>
            </table>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:10}}>
            <button onClick={addLine} style={{background:T.success+"22",color:T.success,border:"1px solid "+T.success+"44",borderRadius:7,padding:"7px 14px",fontWeight:700,cursor:"pointer",fontSize:12}}>+ Ajouter une ligne</button>
            {totalSaisie>0&&<div style={{fontWeight:700,color:T.primary,fontSize:13}}>Total : {fmt(totalSaisie)}</div>}
          </div>
        </>}
      </Modal>}
    </div>}
...
function Debourse({sessions,taches,ch,reload,T,isMobile}){
  ...
  function delSession(id){if(!window.confirm("Supprimer ce projet ?"))return;sb("debourse_taches").eq("session_id",id).del().then(()=>sb("debourse_sessions").eq("id",id).del().then(()=>{setSelSid(null);reload();}));}
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    <Card title="Projets de Débours Sec" action={<button onClick={()=>setShowNew(true)} style={{background:T.primary,color:"#fff",border:"none",borderRadius:7,padding:"6px 12px",fontWeight:700,cursor:"pointer",fontSize:12}}>+ Nouveau</button>} T={T}>
      {sessions.length===0?<Empty msg="Aucun projet" icon="🔢"/>:<div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4}}>
        {sessions.map(s=>{
          const sts=taches.filter(t=>t.session_id===s.id);
          const tot=sts.filter(t=>t.niveau===1).reduce((a,t)=>a+(t.prix_vente_total||0),0);
          ...
          return <div key={s.id} style={{background:active?T.primary+"22":T.mid,border:"2px solid "+(active?T.primary:T.border),borderRadius:10,padding:"10px 14px",minWidth:170,flexShrink:0}}>
            {editNom===s.id?<div style={{display:"flex",flexDirection:"column",gap:5}}>
              <input value={newNom} onChange={e=>setNewNom(e.target.value)} autoFocus style={{background:T.mid,border:"1px solid "+T.primary,borderRadius:5,padding:"4px 7px",color:T.white,fontSize:12,outline:"none"}} onKeyDown={e=>{if(e.key==="Enter")renameSession(s.id);if(e.key==="Escape")setEditNom(null);}}/>
              <div style={{display:"flex",gap:4}}>
                <button onClick={()=>renameSession(s.id)} style={{flex:1,background:T.success,color:"#fff",border:"none",borderRadius:5,padding:"3px",fontSize:10,cursor:"pointer"}}>✔ OK</button>
                <button onClick={()=>setEditNom(null)} style={{background:T.mid,color:T.muted,border:"none",borderRadius:5,padding:"3px 6px",fontSize:10,cursor:"pointer"}}>✕</button>
              </div>
            </div>:<>
              <div onClick={()=>setSelSid(s.id)} style={{fontWeight:700,fontSize:12,color:active?T.primary:T.white,marginBottom:3,cursor:"pointer"}}>{s.nom}</div>
              ...
              <div style={{display:"flex",gap:3,marginTop:7}}>
                <button onClick={()=>setSelSid(s.id)} style={{flex:1,background:T.secondary+"22",color:T.secondary,border:"none",borderRadius:5,padding:"3px",fontSize:9,cursor:"pointer"}}>Ouvrir</button>
                <button onClick={()=>{setEditNom(s.id);setNewNom(s.nom);}} title="Renommer" style={{background:T.warning+"22",color:T.warning,border:"none",borderRadius:5,padding:"3px 7px",fontSize:11,cursor:"pointer"}}>✏️</button>
                <button onClick={()=>delSession(s.id)} style={{background:T.danger+"22",color:T.danger,border:"none",borderRadius:5,padding:"3px 6px",fontSize:9,cursor:"pointer"}}>🗑</button>
              </div>
            </>}
          </div>;
        })}
...
function DebourseEditor({sess,taches:rawTaches,ch,reload,T,isMobile,updateCfg}){
  ...
  const cats2=taches.filter(t=>t.niveau===2);
  ...
  const [saving,setSaving]=useState(false);
  // ── renommer une ligne (libellé catégorie ou ss-cat)
  ...
  async function saveEdit(id,row){
    const c=computeLine(row.mo_u,row.mat_u,row.autres_u,row.quantite,cfg);
    await sb("debourse_taches").eq("id",id).update({libelle:row.libelle,unite:row.unite,quantite:parseFloat(row.quantite)||0,pu:parseFloat(row.pu)||0,mo_u:parseFloat(row.mo_u)||0,mat_u:parseFloat(row.mat_u)||0,autres_u:parseFloat(row.autres_u)||0,debourse_sec_u:Math.round(c.ds),prix_vente_u:Math.round(c.pv_u),prix_vente_total:Math.round(c.pvt)});
    setEditId(null);setEditRow({});reload();
  }
  // Renommer une ligne (cat/ss-cat) sans recalcul
  ...
  async function addLigne(){
    ...
    const niv=parseInt(addForm.niveau);
    let parent_id=null;
    if(niv>=2){const cat=taches.find(t=>t.niveau===1&&t.libelle===addForm.cat_parent);parent_id=cat?cat.id:null;}
    if(niv===3&&addForm.ss_parent){const ss=taches.find(t=>t.niveau===2&&t.libelle===addForm.ss_parent);if(ss)parent_id=ss.id;}
    const c=niv===3?computeLine(addForm.mo_u,addForm.mat_u,addForm.autres_u,addForm.quantite,cfg):{ds:0,pv_u:0,pvt:0};
    await sb("debourse_taches").insert({session_id:sess.id,libelle:addForm.libelle,unite:addForm.unite||"U",quantite:parseFloat(addForm.quantite)||0,pu:parseFloat(addForm.pu)||0,niveau:niv,parent_id,ordre:rawTaches.length+1,mo_u:parseFloat(addForm.mo_u)||0,mat_u:parseFloat(addForm.mat_u)||0,autres_u:parseFloat(addForm.autres_u)||0,debourse_sec_u:Math.round(c.ds),prix_vente_u:Math.round(c.pv_u),prix_vente_total:Math.round(c.pvt)});
  ...
  const iS={background:T.bg,border:"1px solid "+T.primary+"66",borderRadius:4,padding:"3px 5px",color:T.white,fontSize:11,outline:"none",width:"100%"};
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:700}}>
          ...
          <tbody>
            {taches.map(t=>{
              ...
              const bgRow=isCat?T.primary+"18":isSS?T.mid+"cc":"transparent";
              const isEdit=editId===t.id;
              const isLblEdit=editLbl===t.id;
              const editC=isEdit?computeLine(editRow.mo_u,editRow.mat_u,editRow.autres_u,editRow.quantite,cfg):null;
              return <tr key={t.id} style={{background:isEdit?T.warning+"11":bgRow,borderBottom:"1px solid "+T.border+"44"}}>
                ...
                {/* Colonne libellé — avec renommage inline pour cat/ss-cat */}
                <td style={{padding:"5px 7px",paddingLeft:isCat?7:isSS?18:32}}>
                  {!isLine&&isLblEdit?
                    <div style={{display:"flex",gap:4,alignItems:"center"}}>
                      <input value={newLbl} onChange={e=>setNewLbl(e.target.value)} autoFocus style={{...iS,minWidth:120}} onKeyDown={e=>{if(e.key==="Enter")saveLabel(t.id);if(e.key==="Escape")setEditLbl(null);}}/>
                      <button onClick={()=>saveLabel(t.id)} style={{background:T.success,color:"#fff",border:"none",borderRadius:4,padding:"3px 7px",fontSize:10,cursor:"pointer"}}>✔</button>
                      <button onClick={()=>setEditLbl(null)} style={{background:T.danger+"22",color:T.danger,border:"none",borderRadius:4,padding:"3px 6px",fontSize:10,cursor:"pointer"}}>✕</button>
                    </div>:
                ...
                {isEdit?<>
                  ...
                  <td style={{padding:"3px 4px",whiteSpace:"nowrap"}}>
                    <button onClick={()=>saveEdit(t.id,editRow)} style={{background:T.success,color:"#fff",border:"none",borderRadius:4,padding:"4px 8px",fontSize:10,cursor:"pointer",marginRight:3,fontWeight:700}}>✔</button>
                    <button onClick={cancelEdit} style={{background:T.danger+"22",color:T.danger,border:"1px solid "+T.danger+"44",borderRadius:4,padding:"4px 7px",fontSize:10,cursor:"pointer"}}>✕</button>
                  </td>
                </>:<>
                  ...
                  <td style={{padding:"3px 4px",whiteSpace:"nowrap"}}>
                    {!isLine&&<button onClick={()=>{setEditLbl(t.id);setNewLbl(t.libelle);}} title="Renommer" style={{background:T.warning+"22",color:T.warning,border:"none",borderRadius:4,padding:"3px 6px",fontSize:10,cursor:"pointer",marginRight:2}}>✏️</button>}
            ...
            <tr style={{background:T.primary+"33",borderTop:"2px solid "+T.primary}}>
              <td colSpan={7} style={{padding:"8px 7px",fontWeight:800,color:T.primary,fontSize:12}}>TOTAL GÉNÉRAL</td>
              <td style={{padding:"8px 7px",textAlign:"right",color:T.warning,fontWeight:800}}>{fmtS(grandDS)}</td>
              <td style={{padding:"8px 7px",textAlign:"right",color:T.success,fontWeight:800,fontSize:13}}>{fmtS(grandTotal)}</td>
              <td/>
            </tr>
          </tbody>
        </table>
    ...
    {showAdd&&<Modal title="Ajouter une ligne" onClose={()=>setShowAdd(false)} onSave={addLigne} T={T}>
      {saving?<Spin/>:<div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        <FG cols={2}>
          ...
          {addForm.niveau===3&&<>
            <FF label="Quantité" type="number" value={addForm.quantite} onChange={v=>setAddForm(p=>({...p,quantite:v}))} T={T}/>
            <FS label="Unité" value={addForm.unite} onChange={v=>setAddForm(p=>({...p,unite:v}))} options={UNITES} T={T}/>
            <FF label="MO / u (XOF)" type="number" value={addForm.mo_u} onChange={v=>setAddForm(p=>({...p,mo_u:v}))} T={T}/>
            <FF label="Matériaux / u (XOF)" type="number" value={addForm.mat_u} onChange={v=>setAddForm(p=>({...p,mat_u:v}))} T={T}/>
            <FF label="Autres / u (XOF)" type="number" value={addForm.autres_u} onChange={v=>setAddForm(p=>({...p,autres_u:v}))} T={T}/>
          </>}
        </FG>
        {prevDS&&addForm.quantite&&<div style={{background:T.warning+"11",border:"1px solid "+T.warning+"33",borderRadius:7,padding:"8px 12px",display:"flex",gap:16,flexWrap:"wrap"}}>
          <span style={{fontSize:11}}><span style={{color:T.muted}}>DS/u: </span><strong style={{color:T.warning}}>{fmtS(Math.round(prevDS.ds))}</strong></span>
          <span style={{fontSize:11}}><span style={{color:T.muted}}>PV total: </span><strong style={{color:T.success}}>{fmtS(Math.round(prevDS.pvt))}</strong></span>
        </div>}
      </div>}
    </Modal>}
  </div>;
...
// ── INTERVENTIONS — avec modification complète ────────────────────────────────
function Interventions({intv,ch,reload,T,isMobile}){
  const [showNew,setShowNew]=useState(false);
  const [editIntv,setEditIntv]=useState(null);
  const [saving,setSaving]=useState(false);
  const [showDep,setShowDep]=useState(null); // id intervention
  ...
  function save(){
    ...
    const payload={titre:form.titre,type:form.type,description:form.description,date_debut:form.date_debut,date_fin:form.date_fin||null,statut:form.statut,responsable:form.responsable,chantier_id:form.chantier_id||null};
    const op=editIntv?sb("interventions").eq("id",editIntv).update(payload):sb("interventions").insert(payload);
    op.then(()=>{setSaving(false);setShowNew(false);setEditIntv(null);reload();});
  }
  ...
  function addDepLine(){setDepLines(prev=>[...prev,newDepLine()]);}
  function removeDepLine(lid){setDepLines(prev=>prev.length>1?prev.filter(l=>l.id!==lid):prev);}
  async function saveDeps(intvId){
    const valid=depLines.filter(l=>l.libelle&&l.montant);
    if(!valid.length)return;setSavingDep(true);
    for(const l of valid){await sb("intervention_depenses").insert({intervention_id:intvId,libelle:l.libelle,categorie:l.categorie,montant:parseFloat(l.montant),date:l.date,note:l.note});}
    setSavingDep(false);setShowDep(null);setDepLines([newDepLine()]);reload();
  }
  function delDep(id){sb("intervention_depenses").eq("id",id).del().then(()=>reload());}
  const enCours=intv.filter(i=>i.statut==="En cours");
  const autres=intv.filter(i=>i.statut!=="En cours");
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <div style={{fontWeight:700,fontSize:14}}>Interventions ({intv.length})</div>
      <button onClick={openNew} style={{background:T.primary,color:"#fff",border:"none",borderRadius:8,padding:"7px 14px",fontWeight:700,cursor:"pointer",fontSize:12}}>+ Nouvelle</button>
    </div>
    {enCours.length>0&&<div><div style={{fontSize:11,color:T.muted,marginBottom:8,fontWeight:600}}>EN COURS ({enCours.length})</div><div style={{display:"flex",flexDirection:"column",gap:8}}>{enCours.map(i=><IntvCard key={i.id} i={i} ch={ch} T={T} del={del} changeSt={changeSt} openEdit={openEdit} openDep={()=>{setShowDep(i.id);setDepLines([newDepLine()]);}} delDep={delDep}/>)}</div></div>}
    {autres.length>0&&<div><div style={{fontSize:11,color:T.muted,marginBottom:8,fontWeight:600}}>HISTORIQUE ({autres.length})</div><div style={{display:"flex",flexDirection:"column",gap:8}}>{autres.map(i=><IntvCard key={i.id} i={i} ch={ch} T={T} del={del} changeSt={changeSt} openEdit={openEdit} openDep={()=>{setShowDep(i.id);setDepLines([newDepLine()]);}} delDep={delDep}/>)}</div></div>}
    {intv.length===0&&<Empty msg="Aucune intervention" icon="🔧"/>}
    {/* Modal créer / modifier */}
    {showNew&&<Modal title={editIntv?"Modifier l'intervention":"Nouvelle intervention"} onClose={()=>{setShowNew(false);setEditIntv(null);}} onSave={save} saveLabel={editIntv?"Mettre à jour":"Créer"} T={T}>
      {saving?<Spin/>:<FG cols={2}>
        ...
        <FS label="Chantier" value={form.chantier_id} onChange={v=>up("chantier_id",v)} options={[["","— Aucun —"],...ch.map(c=>[c.id,c.nom])]} T={T}/>
        ...
        <FF label="Description" value={form.description} onChange={v=>up("description",v)} rows={3} full T={T}/>
      </FG>}
    </Modal>}
    {/* Modal dépenses multi-lignes pour intervention */}
    {showDep&&(()=>{
      const intvObj=intv.find(i=>i.id===showDep);
      if(!intvObj)return null;
      const totalS=depLines.reduce((a,l)=>a+(parseFloat(l.montant)||0),0);
      return <Modal title={"Dépenses — "+intvObj.titre} onClose={()=>setShowDep(null)} onSave={()=>saveDeps(showDep)} saveLabel={"Enregistrer "+depLines.filter(l=>l.libelle&&l.montant).length+" ligne(s)"} T={T} wide>
        {savingDep?<Spin/>:<>
          {intvObj.depenses.length>0&&<div style={{marginBottom:14}}>
            <div style={{fontSize:11,color:T.muted,marginBottom:6,fontWeight:600}}>Dépenses existantes</div>
            {intvObj.depenses.map(d=><div key={d.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid "+T.border+"44",fontSize:12}}><div><span style={{fontWeight:600}}>{d.libelle}</span><span style={{color:T.muted,marginLeft:8}}>{d.date}</span></div><div style={{display:"flex",gap:6,alignItems:"center"}}><Badge label={d.categorie} color={catC(d.categorie,T)} small/><span style={{color:T.primary,fontWeight:700}}>{fmt(d.montant)}</span><button onClick={()=>delDep(d.id)} style={{background:T.danger+"22",color:T.danger,border:"none",borderRadius:4,padding:"2px 6px",fontSize:10,cursor:"pointer"}}>✕</button></div></div>)}
          </div>}
          <div style={{fontSize:11,color:T.muted,marginBottom:6,fontWeight:600}}>Nouvelles lignes</div>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}>
              <thead><tr style={{background:T.mid}}>{["Libellé *","Catégorie","Montant *","Date","Note",""].map((h,i)=><th key={i} style={{padding:"7px 8px",textAlign:"left",fontSize:10,color:T.muted,fontWeight:600}}>{h}</th>)}</tr></thead>
              <tbody>{depLines.map(l=>{
                const iS={background:T.bg,border:"1px solid "+T.border,borderRadius:5,padding:"6px 8px",color:T.white,fontSize:12,outline:"none",width:"100%"};
                return <tr key={l.id} style={{borderBottom:"1px solid "+T.border+"44"}}>
                  <td style={{padding:"4px 4px"}}><input value={l.libelle} onChange={e=>upDepLine(l.id,"libelle",e.target.value)} placeholder="ex: Pièce de rechange" style={iS}/></td>
                  <td style={{padding:"4px 4px"}}><select value={l.categorie} onChange={e=>upDepLine(l.id,"categorie",e.target.value)} style={iS}>{CATS.map(c=><option key={c} value={c}>{c}</option>)}</select></td>
                  <td style={{padding:"4px 4px"}}><input type="number" value={l.montant} onChange={e=>upDepLine(l.id,"montant",e.target.value)} placeholder="0" style={{...iS,width:110}}/></td>
                  <td style={{padding:"4px 4px"}}><input type="date" value={l.date} onChange={e=>upDepLine(l.id,"date",e.target.value)} style={{...iS,width:130}}/></td>
                  <td style={{padding:"4px 4px"}}><input value={l.note} onChange={e=>upDepLine(l.id,"note",e.target.value)} placeholder="Optionnel" style={iS}/></td>
                  <td style={{padding:"4px 4px"}}><button onClick={()=>removeDepLine(l.id)} style={{background:T.danger+"22",color:T.danger,border:"none",borderRadius:5,padding:"5px 8px",cursor:"pointer",fontSize:13}}>✕</button></td>
                </tr>;
              })}</tbody>
            </table>
          </div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:10}}>
            <button onClick={addDepLine} style={{background:T.success+"22",color:T.success,border:"1px solid "+T.success+"44",borderRadius:7,padding:"7px 14px",fontWeight:700,cursor:"pointer",fontSize:12}}>+ Ajouter une ligne</button>
  ...
  return <div style={{background:T.card,border:"1px solid "+T.border,borderRadius:T.borderRadius,padding:12}}>
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:6,flexWrap:"wrap",gap:6}}>
      <div style={{flex:1,cursor:"pointer"}} onClick={()=>setExpanded(e=>!e)}>
        <div style={{fontWeight:700,fontSize:13}}>{i.titre} <span style={{color:T.muted,fontSize:11}}>{expanded?"▲":"▼"}</span></div>
        {chNom&&<div style={{fontSize:11,color:T.muted}}>{chNom}</div>}
      </div>
      <div style={{display:"flex",gap:4,alignItems:"center",flexWrap:"wrap"}}>
        <Badge label={i.statut||"En cours"} color={intStC(i.statut,T)}/>
        <button onClick={()=>openEdit(i)} title="Modifier" style={{background:T.warning+"22",color:T.warning,border:"none",borderRadius:5,padding:"3px 7px",fontSize:11,cursor:"pointer"}}>✏️</button>
        <button onClick={()=>openDep()} title="Dépenses" style={{background:T.primary+"22",color:T.primary,border:"none",borderRadius:5,padding:"3px 7px",fontSize:11,cursor:"pointer"}}>💸</button>
    ...
    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
      ...
      {i.date_debut&&<span style={{fontSize:10,color:T.muted}}>Début: {i.date_debut}</span>}
      {i.date_fin&&<span style={{fontSize:10,color:T.muted}}>Fin: {i.date_fin}</span>}
      {i.responsable&&<span style={{fontSize:10,color:T.secondary}}>👤 {i.responsable}</span>}
      {cost>0&&<span style={{fontSize:10,fontWeight:700,color:T.primary}}>{fmt(cost)}</span>}
    </div>
    {expanded&&i.description&&<div style={{marginTop:8,fontSize:12,color:T.muted,borderTop:"1px solid "+T.border,paddingTop:8}}>{i.description}</div>}
    {expanded&&i.depenses.length>0&&<div style={{marginTop:8,borderTop:"1px solid "+T.border,paddingTop:8}}>
      <div style={{fontSize:10,color:T.muted,fontWeight:600,marginBottom:4}}>DÉPENSES ({i.depenses.length})</div>
      {i.depenses.map(d=><div key={d.id} style={{display:"flex",justifyContent:"space-between",fontSize:11,padding:"4px 0",borderBottom:"1px solid "+T.border+"33"}}><span>{d.libelle} <span style={{color:T.muted}}>— {d.date}</span></span><span style={{color:T.primary,fontWeight:700}}>{fmt(d.montant)}</span></div>)}
    </div>}
  </div>;
}
...
function KpiPage({ch,intv,T,isMobile}){
  const totalB=ch.reduce((a,c)=>a+c.budgetInitial,0);
  const totalD=ch.reduce((a,c)=>a+totalDep(c),0);
  const enDerive=ch.filter(c=>c.statut==="En derive").length;
  const taux=pct(totalD,totalB);
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)",gap:8}}>
      ...
      <Kpi icon="📊" label="Taux consommation" value={taux+"%"} color={taux>80?T.danger:T.success} T={T} compact/>
    ...
    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:14}}>
      ...
      <Card title="Répartition des dépenses" T={T}>{catData.length>0?<ResponsiveContainer width="100%" height={180}><PieChart><Pie data={catData} dataKey="value" cx="50%" cy="50%" outerRadius={70} label={e=>e.name.slice(0,8)}>{catData.map((_,i)=><Cell key={i} fill={COLORS[i%COLORS.length]}/>)}</Pie><Tooltip formatter={v=>fmt(v)} contentStyle={{background:T.card,border:"1px solid "+T.border,color:T.white}}/></PieChart></ResponsiveContainer>:<Empty msg="Aucune dépense" icon="📊"/>}</Card>
    </div>
  </div>;
}

// ── IA ────────────────────────────────────────────────────────────────────────
function IA({ch,intv,T}){
  const [prompt,setPrompt]=useState("");
  const [resp,setResp]=useState("");
  const [loading,setLoading]=useState(false);
  async function ask(){
    if(!prompt.trim())return;setLoading(true);setResp("");
    const ctx=`Tu es un assistant BTP expert. Données chantiers: ${JSON.stringify(ch.map(c=>({nom:c.nom,statut:c.statut,budget:c.budgetInitial,depenses:totalDep(c),pct:pct(totalDep(c),c.budgetInitial)})))}.`;
    try{const r=await aiCall({model:"claude-sonnet-4-20250514",max_tokens:1000,system:ctx,messages:[{role:"user",content:prompt}]});setResp((r.content||[]).map(b=>b.text||"").join(""));}catch(e){setResp("Erreur: "+e.message);}
    setLoading(false);
  }
  const sug=["Quels chantiers sont en dérive ?","Résumé de l'activité","Recommandations ?","Analyse dépenses par catégorie"];
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    <Card title="🤖 Assistant IA BTP" T={T}>
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>{sug.map(s=><button key={s} onClick={()=>setPrompt(s)} style={{background:T.secondary+"22",color:T.secondary,border:"1px solid "+T.secondary+"44",borderRadius:20,padding:"5px 10px",fontSize:11,cursor:"pointer"}}>{s}</button>)}</div>
      <FF label="Votre question" value={prompt} onChange={setPrompt} rows={3} full T={T} placeholder="Posez une question sur vos chantiers..."/>
      <button onClick={ask} disabled={loading} style={{marginTop:10,background:T.primary,color:"#fff",border:"none",borderRadius:8,padding:"9px 18px",fontWeight:700,cursor:"pointer",fontSize:13,opacity:loading?0.6:1}}>{loading?"⏳ Analyse...":"🤖 Analyser"}</button>
    </Card>
    {resp&&<Card title="Réponse" T={T}><div style={{fontSize:13,lineHeight:1.7,whiteSpace:"pre-wrap",color:T.white}}>{resp}</div></Card>}
  </div>;
}