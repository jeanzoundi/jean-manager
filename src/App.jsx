...
// ── Sous-statuts automatiques basés sur les dépenses ─────────────────────────
...
function exportDepenses(chantiers, scope, chantierId) {
  ...
  const total = target.flatMap(c=>c.depenses).reduce((a,d)=>a+d.montant,0);
  ...
  CATEGORIES.forEach(cat=>{const l=target.flatMap(c=>c.depenses.filter(d=>d.categorie===cat));const t=l.reduce((a,d)=>a+d.montant,0);if(t>0)d2.push([cat,t,l.length,parseFloat((t/total*100).toFixed(1))])});
...
// ── Données ───────────────────────────────────────────────────────────────────
const initInterventions = [
  {id:1,titre:"Fissure mur porteur",description:"Fissure mur porteur côté nord, expertise requise",type:"Urgence",intervenant:"Bureau Structura",chantier:"Réhabilitation Hôtel Atlas",client:"Atlas Hotels Group",dateCreation:"2025-02-14",duree:3,statut:"En cours",facturee:false,
   depenses:[{id:1,libelle:"Expertise structure",categorie:"Sous-traitance",montant:200000,date:"2025-02-14",note:""},{id:2,libelle:"Main d'œuvre urgence",categorie:"Main d'œuvre",montant:150000,date:"2025-02-15",note:""}],
   todos:[{id:1,texte:"Contacter ingénieur structure",fait:true},{id:2,texte:"Rédiger rapport expertise",fait:false},{id:3,texte:"Informer le client",fait:false}]},
  {id:2,titre:"Maintenance pompes béton",description:"Révision périodique pompes à béton",type:"Préventive",intervenant:"Tech BTP Services",chantier:"Centre Commercial Agora",client:"Foncière Agora SARL",dateCreation:"2025-02-10",duree:1,statut:"Terminée",facturee:true,
   depenses:[{id:1,libelle:"Main d'œuvre techniciens",categorie:"Main d'œuvre",montant:80000,date:"2025-02-10",note:""},{id:2,libelle:"Pièces de rechange",categorie:"Matériaux",montant:40000,date:"2025-02-10",note:""}],
   todos:[{id:1,texte:"Vérifier pompe A",fait:true},{id:2,texte:"Vérifier pompe B",fait:true}]},
  {id:3,titre:"Remplacement canalisation",description:"Canalisation endommagée zone B",type:"Corrective",intervenant:"Plomberie CI",chantier:"Résidence Les Acacias",client:"Groupe Immobilier",dateCreation:"2025-02-16",duree:2,statut:"En attente",facturee:false,
   depenses:[],
   todos:[{id:1,texte:"Commander les tuyaux",fait:false},{id:2,texte:"Planifier intervention",fait:false}]},
  {id:4,titre:"Inspection électrique",description:"Contrôle conformité installations",type:"Inspection",intervenant:"SOCOTEC",chantier:"Villa Prestige – Cocody",client:"M. Karim Benali",dateCreation:"2025-02-20",duree:1,statut:"En attente",facturee:false,
   depenses:[],
   todos:[{id:1,texte:"Préparer plan électrique",fait:false}]},
];

const initChantiers = [
  {id:1,nom:"Résidence Les Acacias",client:"Groupe Immobilier",localisation:"Abidjan, CI",lat:5.36,lng:-4.0083,type:"Construction",budgetInitial:28000000,dateDebut:"2024-03-01",dateFin:"2025-06-30",statut:"En cours",alertes:["Budget à 75% consommé"],
   depenses:[{id:1,libelle:"Salaires ouvriers S1",categorie:"Main d'œuvre",montant:850000,date:"2025-02-03",note:""},{id:2,libelle:"Achat ciment 200 sacs",categorie:"Matériaux",montant:320000,date:"2025-02-05",note:""},{id:3,libelle:"Location grue mobile",categorie:"Équipement",montant:500000,date:"2025-02-08",note:""}],score:82},
  {id:2,nom:"Réhabilitation Hôtel Atlas",client:"Atlas Hotels Group",localisation:"Bouaké, CI",lat:7.69,lng:-5.03,type:"Réhabilitation",budgetInitial:12000000,dateDebut:"2024-01-15",dateFin:"2025-02-28",statut:"En dérive",alertes:["⚠️ Dépassement budget","Facture impayée"],
   depenses:[{id:1,libelle:"Sous-traitant peinture",categorie:"Sous-traitance",montant:1200000,date:"2025-01-18",note:""},{id:2,libelle:"Transport matériaux",categorie:"Transport",montant:180000,date:"2025-01-22",note:""},{id:3,libelle:"Main d'œuvre finitions",categorie:"Main d'œuvre",montant:11000000,date:"2025-02-01",note:""}],score:41},
  {id:3,nom:"Centre Commercial Agora",client:"Foncière Agora SARL",localisation:"Yamoussoukro, CI",lat:6.8276,lng:-5.2893,type:"Construction",budgetInitial:85000000,dateDebut:"2024-09-01",dateFin:"2026-03-31",statut:"En cours",alertes:[],
   depenses:[{id:1,libelle:"Terrassement zone A",categorie:"Équipement",montant:12000000,date:"2024-10-01",note:""},{id:2,libelle:"Béton fondations",categorie:"Matériaux",montant:18000000,date:"2024-11-15",note:""}],score:91},
  {id:4,nom:"Maintenance Usine Sofac",client:"Sofac Industries",localisation:"San-Pédro, CI",lat:4.7485,lng:-6.6363,type:"Maintenance",budgetInitial:4500000,dateDebut:"2024-11-01",dateFin:"2025-01-31",statut:"Clôturé",alertes:[],
   depenses:[{id:1,libelle:"Techniciens spécialisés",categorie:"Main d'œuvre",montant:900000,date:"2024-11-10",note:""},{id:2,libelle:"Pièces détachées",categorie:"Matériaux",montant:2100000,date:"2024-11-15",note:""}],score:88},
  {id:5,nom:"Villa Prestige – Cocody",client:"M. Karim Benali",localisation:"Abidjan, CI",lat:5.378,lng:-3.99,type:"Construction",budgetInitial:6500000,dateDebut:"2025-02-01",dateFin:"2025-12-31",statut:"Planifié",alertes:[],
   depenses:[],score:95},
];
...
const Modal=({title,onClose,onSave,children,wide})=>(
  <div style={{position:"fixed",inset:0,background:"#00000099",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div style={{background:C.dark,border:"1px solid "+C.border,borderRadius:16,padding:28,width:"100%",maxWidth:wide?720:560,maxHeight:"90vh",overflow:"auto"}}>
...
// ═════════════════════════════════════════════════════════════════════════════
// APP avec sauvegarde automatique
// ═════════════════════════════════════════════════════════════════════════════
export default function App() {
  // Chargement depuis le stockage persistant
  const [chantiers,setChantiers]=useState(()=>{
    const saved=localStorage.getItem('jean_manager_chantiers');
    return saved ? JSON.parse(saved) : initChantiers;
  });
  const [interventions,setInterventions]=useState(()=>{
    const saved=localStorage.getItem('jean_manager_interventions');
    return saved ? JSON.parse(saved) : initInterventions;
  });
  ...
  useState(()=>{
    localStorage.setItem('jean_manager_chantiers',JSON.stringify(chantiers));
  },[chantiers]);
  
  useState(()=>{
    localStorage.setItem('jean_manager_interventions',JSON.stringify(interventions));
  },[interventions]);

  const selected=chantiers.find(c=>c.id===selectedId);
  const openChantier=(id)=>{setSelectedId(id);setPage("fiche");setOnglet("infos");};
  const deleteChantier=(id)=>{setChantiers(p=>p.filter(c=>c.id!==id));setPage("chantiers");};
  const saveChantier=()=>{
    if(!newForm.nom||!newForm.client||!newForm.budgetInitial)return;
    setChantiers(p=>[...p,{...newForm,id:Date.now(),statut:"Brouillon",alertes:[],depenses:[],score:100,lat:5.35,lng:-4.0,budgetInitial:parseFloat(newForm.budgetInitial)}]);
    setShowNew(false);setNewForm({nom:"",client:"",localisation:"",type:"Construction",budgetInitial:"",dateDebut:"",dateFin:""});
  };

  // Fonction de réinitialisation complète
  const resetAllData=()=>{
    if(window.confirm("⚠️ ATTENTION : Voulez-vous vraiment réinitialiser TOUTES les données ?\n\nCette action est irréversible et supprimera :\n- Tous les chantiers\n- Toutes les dépenses\n- Toutes les interventions\n\nLes données de démonstration seront restaurées.")){
      localStorage.removeItem('jean_manager_chantiers');
      localStorage.removeItem('jean_manager_interventions');
      setChantiers(initChantiers);
      setInterventions(initInterventions);
      setPage("dashboard");
  ...
  const titles={dashboard:"📊 Dashboard Global",chantiers:"🏗️ Mes Chantiers",interventions:"🔧 Interventions",alertes:"🔔 Alertes",kpi:"📈 KPIs & Analyses",ia:"🤖 Intelligence IA",gestion:"⚙️ Gestion des projets",fiche:selected?"🏗️ "+selected.nom:""};
    ...
    <div style={{display:"flex",height:"100vh",background:C.bg,color:C.white,fontFamily:"'Segoe UI',system-ui,sans-serif",overflow:"hidden"}}>
      ...
      <div style={{width:sideOpen?220:60,background:C.dark,borderRight:"1px solid "+C.border,display:"flex",flexDirection:"column",transition:"width .25s",overflow:"hidden",flexShrink:0}}>
        <div style={{padding:"20px 14px 16px",borderBottom:"1px solid "+C.border,display:"flex",alignItems:"center",gap:10}}>
          ...
          {sideOpen&&<div><div style={{fontWeight:700,fontSize:14}}>JEAN MANAGER</div><div style={{fontSize:10,color:C.orange}}>ADMIN</div></div>}
        </div>
        ...
        {sideOpen&&(
          <div style={{padding:"8px",borderTop:"1px solid "+C.border}}>
            <button onClick={resetAllData} style={{width:"100%",background:C.red+"22",border:"1px solid "+C.red+"44",color:C.red,borderRadius:8,padding:"8px",fontSize:11,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
              🔄 Réinitialiser
            </button>
          </div>
        )}
      ...
      <div style={{flex:1,overflow:"auto",display:"flex",flexDirection:"column"}}>
        <div style={{background:C.dark,borderBottom:"1px solid "+C.border,padding:"12px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          ...
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            {(page==="chantiers"||page==="dashboard")&&<>
              <button onClick={()=>exportDepenses(chantiers,"all")} style={{background:C.green+"22",color:C.green,border:"1px solid "+C.green+"55",borderRadius:8,padding:"8px 14px",fontWeight:700,cursor:"pointer",fontSize:13}}>📥 Export Dépenses</button>
            ...
            {page==="fiche"&&selected&&<>
              <button onClick={()=>exportDepenses(chantiers,"one",selected.id)} style={{background:C.green+"22",color:C.green,border:"1px solid "+C.green+"55",borderRadius:8,padding:"8px 14px",fontWeight:700,cursor:"pointer",fontSize:13}}>📥 Exporter dépenses</button>
              <button onClick={()=>{if(window.confirm("Supprimer ce chantier ?"))deleteChantier(selected.id);}} style={{background:C.red+"22",color:C.red,border:"1px solid "+C.red+"44",borderRadius:8,padding:"8px 14px",fontWeight:700,cursor:"pointer",fontSize:13}}>🗑️ Supprimer</button>
            </>}
        ...
        <div style={{flex:1,overflow:"auto",padding:24}}>
          ...
          {page==="fiche"&&selected&&<FichePage chantier={selected} onglet={onglet} setOnglet={setOnglet} setPage={setPage} chantiers={chantiers} setChantiers={setChantiers}/>}
          {page==="interventions"&&<InterventionsPage interventions={interventions} setInterventions={setInterventions} chantiers={chantiers}/>}
          ...
          {page==="gestion"&&<GestionPage chantiers={chantiers} setChantiers={setChantiers} openChantier={openChantier}/>}
        </div>
      </div>
...
// ─── helpers partagés ─────────────────────────────────────────────────────────
const totalDep = c => (c.depenses||[]).reduce((a,d)=>a+d.montant,0);
// budgetConsomme est toujours calculé depuis les dépenses réelles
const getBudgetConsomme = c => totalDep(c);

// Génération automatique des alertes basées sur les dépenses réelles
const genAlertes = (chantiers) => {
  const alertes=[];
  chantiers.forEach(c=>{
    const dep=getBudgetConsomme(c);
    const p=pct(dep,c.budgetInitial);
    if(p>100) alertes.push({niveau:"critique",msg:"Dépassement budget : "+p+"% consommé ("+fmt(dep)+" / "+fmt(c.budgetInitial)+")",chantier:c});
    else if(p>=90) alertes.push({niveau:"danger",msg:"Budget à "+p+"% consommé — risque de dépassement imminent",chantier:c});
    ...
    if(c.dateFin){
      ...
      else if(diffDays>=0&&diffDays<=7&&c.statut!=="Clôturé") alertes.push({niveau:"warning",msg:"Échéance dans "+diffDays+" jour(s) — clôture à préparer",chantier:c});
    }
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:24}}>
      <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
        <KpiCard icon="🏗️" label="Chantiers" value={total} color={C.orange}/>
        <KpiCard icon="💰" label="Budget total" value={fmt(totalB)} sub={fmt(totalD)+" dépensé"}/>
        <KpiCard icon="📊" label="Taux consommation" value={pct(totalD,totalB)+"%"} color={pct(totalD,totalB)>100?C.red:pct(totalD,totalB)>80?C.yellow:C.green}/>
...
function ChantierCard({c,onClick,onDelete}){
  ...
  const ssb=getSousStatutBudget(d,c.budgetInitial);
  return(
    <div onClick={onClick} style={{background:C.card,border:"1px solid "+(ssb==="Dépassement"?C.red+"66":C.border),borderRadius:14,padding:20,cursor:"pointer",position:"relative"}}
      ...
      <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>
        ...
        <Badge label={ssb} color={getSousStatutBudget(d,c.budgetInitial)==="Conforme"?C.green:getSousStatutBudget(d,c.budgetInitial)==="80% consommé"?C.yellow:C.red} small/>
      </div>
...
// FICHE
// ═════════════════════════════════════════════════════════════════════════════
function FichePage({chantier:c,onglet,setOnglet,setPage,chantiers,setChantiers}){
  const onglets=["infos","dashboard","dépenses","analyse"];
  const dep=totalDep(c);
  const depPct=pct(dep,c.budgetInitial);
  const ssb=getSousStatutBudget(dep,c.budgetInitial);
  ...
  const changeStatut=s=>{setChantiers(p=>p.map(ch=>ch.id===c.id?{...ch,statut:s}:ch));setShowStatutMenu(false);};
  const addDep=()=>{
    if(!depForm.libelle||!depForm.montant)return;
    setChantiers(p=>p.map(ch=>ch.id===c.id?{...ch,depenses:[...ch.depenses,{id:Date.now(),...depForm,montant:parseFloat(depForm.montant)}]}:ch));
    setShowNewDep(false);setDepForm({libelle:"",categorie:"Main d'œuvre",montant:"",date:new Date().toISOString().slice(0,10),note:""});
  };
  const delDep=id=>setChantiers(p=>p.map(ch=>ch.id===c.id?{...ch,depenses:ch.depenses.filter(d=>d.id!==id)}:ch));
  const saveEditDep=()=>{
    setChantiers(p=>p.map(ch=>ch.id===c.id?{...ch,depenses:ch.depenses.map(d=>d.id===editDepId?{...d,...editDepData,montant:parseFloat(editDepData.montant)}:d)}:ch));
    setEditDepId(null);
  };

  const filteredDep=filterCat==="Toutes"?c.depenses:c.depenses.filter(d=>d.categorie===filterCat);
  const depParCat=CATEGORIES.map(cat=>({cat,total:c.depenses.filter(d=>d.categorie===cat).reduce((a,d)=>a+d.montant,0)})).filter(x=>x.total>0);
    ...
    <div style={{display:"flex",flexDirection:"column",gap:0}}>
            ...
            <div style={{position:"relative"}}>
              ...
              {showStatutMenu&&(
                <div style={{position:"absolute",right:0,top:"calc(100% + 6px)",background:C.dark,border:"1px solid "+C.border,borderRadius:10,zIndex:50,minWidth:170,overflow:"hidden",boxShadow:"0 8px 24px #0008"}}>
                  <div style={{padding:"8px 12px",fontSize:11,color:C.muted,borderBottom:"1px solid "+C.border}}>Changer le statut</div>
      ...
      {onglet==="dashboard"&&(
        <div style={{display:"flex",flexDirection:"column",gap:20}}>
          ...
          {c.alertes.length>0&&<div style={{background:C.red+"11",border:"1px solid "+C.red+"44",borderRadius:12,padding:16}}><div style={{fontWeight:700,color:C.red,marginBottom:10}}>⚠️ Alertes</div>{c.alertes.map((a,i)=><div key={i} style={{fontSize:13,padding:"4px 0"}}>• {a}</div>)}</div>}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>exportDepenses(chantiers,"one",c.id)} style={{background:C.green+"22",color:C.green,border:"1px solid "+C.green+"55",borderRadius:8,padding:"8px 14px",fontWeight:700,cursor:"pointer",fontSize:13}}>📥 Exporter</button>
          ...
          {filteredDep.length>0&&<div style={{background:C.card,border:"1px solid "+C.orange+"44",borderRadius:10,padding:"12px 18px",display:"flex",justifyContent:"space-between"}}>
            ...
            <span style={{fontWeight:800,color:C.orange,fontSize:16}}>{fmt(filteredDep.reduce((a,d)=>a+d.montant,0))}</span>
          </div>}
          {showNewDep&&(
...
// INTERVENTIONS
// ═════════════════════════════════════════════════════════════════════════════
function InterventionsPage({interventions,setInterventions,chantiers}){
  const [showNew,setShowNew]=useState(false);
  ...
  const [form,setForm]=useState({titre:"",description:"",type:"Corrective",intervenant:"",chantier:"",client:"",dateCreation:new Date().toISOString().slice(0,10),duree:"",statut:"En attente",facturee:false});
  ...
  const totalNonFacture=totalCout-totalFacture;

  const saveNew=()=>{
    if(!form.titre)return;
    setInterventions(p=>[...p,{...form,id:Date.now(),depenses:[],todos:[],duree:parseInt(form.duree)||1}]);
    setShowNew(false);
    setForm({titre:"",description:"",type:"Corrective",intervenant:"",chantier:"",client:"",dateCreation:new Date().toISOString().slice(0,10),duree:"",statut:"En attente",facturee:false});
  };
  const updateStatut=(id,s)=>setInterventions(p=>p.map(i=>i.id===id?{...i,statut:s}:i));
  const toggleFacturee=(id)=>setInterventions(p=>p.map(i=>i.id===id?{...i,facturee:!i.facturee}:i));
  const deleteInt=(id)=>setInterventions(p=>p.filter(i=>i.id!==id));
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
        ...
        <KpiCard icon="❌" label="Non facturé" value={fmt(totalNonFacture)} color={C.red}/>
      </div>
      ...
      <div style={{background:C.card,border:"1px solid "+C.border,borderRadius:12,padding:"14px 18px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div style={{display:"flex",gap:12,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <span style={{fontSize:12,color:C.muted}}>Trier par :</span>
          ...
          <button onClick={()=>setShowNew(true)} style={{marginLeft:"auto",background:C.orange,color:"#fff",border:"none",borderRadius:8,padding:"8px 18px",fontWeight:700,cursor:"pointer",fontSize:13}}>+ Nouvelle intervention</button>
        </div>
      </div>
        ...
        {filtered.map(i=>(
          <div key={i.id} style={{background:C.card,border:"1px solid "+(i.type==="Urgence"?C.red+"66":C.border),borderRadius:14,padding:18,display:"flex",flexDirection:"column",gap:12}}>
            ...
            <div style={{display:"flex",gap:10,alignItems:"center",background:C.mid,borderRadius:8,padding:"10px 14px"}}>
              <div style={{flex:1}}>
                <div style={{fontSize:10,color:C.muted}}>Coût total dépenses</div>
              ...
              <button onClick={()=>toggleFacturee(i.id)} style={{display:"flex",alignItems:"center",gap:6,background:i.facturee?C.green+"22":C.red+"22",border:"1px solid "+(i.facturee?C.green:C.red)+"55",borderRadius:8,padding:"6px 14px",color:i.facturee?C.green:C.red,fontWeight:700,fontSize:12,cursor:"pointer"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                <span style={{fontSize:11,color:C.muted}}>Statut :</span>
      ...
      {showNew&&(
        <Modal title="🔧 Nouvelle Intervention" onClose={()=>setShowNew(false)} onSave={saveNew}>
          <FGrid>
            ...
            <FField label="Date création" type="date" value={form.dateCreation} onChange={v=>setForm(p=>({...p,dateCreation:v}))}/>
      ...
      {viewId&&<IntervDetail intervention={interventions.find(i=>i.id===viewId)} setInterventions={setInterventions} onClose={()=>setViewId(null)}/>}
    </div>
  );
}

const totalIntDep=i=>(i.depenses||[]).reduce((a,d)=>a+d.montant,0);

function IntervDetail({intervention:i,setInterventions,onClose}){
  ...
  const addDep=()=>{
    if(!depForm.libelle||!depForm.montant)return;
    setInterventions(p=>p.map(x=>x.id===i.id?{...x,depenses:[...x.depenses,{id:Date.now(),...depForm,montant:parseFloat(depForm.montant)}]}:x));
    setShowDep(false);setDepForm({libelle:"",categorie:"Main d'œuvre",montant:"",date:new Date().toISOString().slice(0,10),note:""});
  };
  const delDep=id=>setInterventions(p=>p.map(x=>x.id===i.id?{...x,depenses:x.depenses.filter(d=>d.id!==id)}:x));
  const addTodo=()=>{
    if(!todoText.trim())return;
    setInterventions(p=>p.map(x=>x.id===i.id?{...x,todos:[...x.todos,{id:Date.now(),texte:todoText,fait:false}]}:x));
    setTodoText("");
  };
  const toggleTodo=id=>setInterventions(p=>p.map(x=>x.id===i.id?{...x,todos:x.todos.map(t=>t.id===id?{...t,fait:!t.fait}:t)}:x));
  const delTodo=id=>setInterventions(p=>p.map(x=>x.id===i.id?{...x,todos:x.todos.filter(t=>t.id!==id)}:x));
      ...
      <div style={{background:C.dark,border:"1px solid "+C.border,borderRadius:16,padding:28,width:"100%",maxWidth:700,maxHeight:"90vh",overflow:"auto"}}>
        ...
        <div style={{display:"flex",gap:6,marginBottom:16}}>
          {["depenses","todos"].map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={{padding:"8px 18px",borderRadius:8,border:"1px solid "+(tab===t?C.orange:C.border),background:tab===t?C.orange:C.card,color:tab===t?"#fff":C.muted,cursor:"pointer",fontWeight:tab===t?700:400,fontSize:13,textTransform:"capitalize"}}>
        ...
        {tab==="depenses"&&(
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            <div style={{display:"flex",justifyContent:"flex-end"}}>
              <button onClick={()=>setShowDep(true)} style={{background:C.orange,color:"#fff",border:"none",borderRadius:8,padding:"8px 16px",fontWeight:700,cursor:"pointer",fontSize:13}}>+ Ajouter dépense</button>
            </div>
            ...
            {i.depenses.map(d=>(
              <div key={d.id} style={{background:C.mid,borderRadius:10,padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                <div>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    <div style={{width:7,height:7,borderRadius:"50%",background:catColor(d.categorie)}}/>
                    <span style={{fontWeight:600,fontSize:13}}>{d.libelle}</span>
                  </div>
            ...
            {i.depenses.length>0&&(
              <div style={{background:C.card,border:"1px solid "+C.orange+"44",borderRadius:10,padding:"12px 16px",display:"flex",justifyContent:"space-between"}}>
                <span style={{fontWeight:700,color:C.muted}}>Total dépenses intervention</span>
                <span style={{fontWeight:800,color:C.orange,fontSize:16}}>{fmt(totalD)}</span>
              </div>
            )}
            {showDep&&(
              <div style={{background:C.card,border:"2px solid "+C.orange,borderRadius:12,padding:16}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            ...
            {i.todos.map(t=>(
              <div key={t.id} style={{display:"flex",alignItems:"center",gap:10,background:C.mid,borderRadius:8,padding:"10px 14px"}}>
                <button onClick={()=>toggleTodo(t.id)} style={{width:22,height:22,borderRadius:"50%",background:t.fait?C.green:C.border,border:"2px solid "+(t.fait?C.green:C.muted),display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,color:"#fff",fontSize:12}}>
                  {t.fait?"✓":""}
                </button>
            ...
            {i.todos.length>0&&(
              <div style={{textAlign:"right",fontSize:12,color:C.muted}}>
                {todosDone}/{i.todos.length} tâche(s) complétée(s)
                <PBar p={pct(todosDone,i.todos.length)} color={C.green} h={4}/>
              </div>
            )}
          </div>
        )}
...
// ═════════════════════════════════════════════════════════════════════════════
// GESTION PROJETS — Onglet suppression & récap
// ═════════════════════════════════════════════════════════════════════════════
function GestionPage({chantiers,setChantiers,openChantier}){
  ...
  const handleDelete=(id)=>{
    setChantiers(p=>p.filter(c=>c.id!==id));
    setConfirmId(null);
  };
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      <Card title="⚙️ Gestion & Suppression des projets">
        <div style={{marginBottom:16}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Rechercher un projet ou client..." style={{width:"100%",background:C.mid,border:"1px solid "+C.border,borderRadius:8,padding:"9px 14px",color:C.white,fontSize:13,boxSizing:"border-box",outline:"none"}}/>
        </div>
              ...
              <div key={c.id} style={{background:C.mid,border:"1px solid "+(confirmId===c.id?C.red+"88":C.border),borderRadius:12,padding:"14px 18px"}}>
                {confirmId===c.id?(
                  ...
                  <div>
                    ...
                    <div style={{fontSize:12,color:C.muted,marginBottom:14}}>Cette action est irréversible. Toutes les dépenses associées seront perdues.</div>
                    <div style={{display:"flex",gap:10}}>
                      <button onClick={()=>setConfirmId(null)} style={{flex:1,padding:"8px",background:C.card,color:C.white,border:"1px solid "+C.border,borderRadius:8,cursor:"pointer",fontWeight:600}}>Annuler</button>
                      <button onClick={()=>handleDelete(c.id)} style={{flex:1,padding:"8px",background:C.red,color:"#fff",border:"none",borderRadius:8,cursor:"pointer",fontWeight:700}}>✔ Supprimer définitivement</button>
                    </div>
                  </div>
                ):(
                  <div style={{display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
                    ...
                    <div style={{flex:2,minWidth:160}}>
                      ...
                      <div style={{fontSize:12,color:C.muted,marginTop:2}}>👤 {c.client} · 📍 {c.localisation}</div>
                    </div>
                    {/* Statuts */}
                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                    ...
                    <div style={{minWidth:200,flex:1}}>
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:C.muted,marginBottom:3}}>
                        <span>Dépenses {fmt(dep)}</span>
                        <span style={{color:p>100?C.red:p>80?C.yellow:C.green,fontWeight:700}}>{p}%</span>
                      </div>
                      <PBar p={p} color={p>100?C.red:p>80?C.yellow:C.green} h={6}/>
                      <div style={{fontSize:10,color:C.muted,marginTop:2}}>Budget : {fmt(c.budgetInitial)}</div>
                    </div>
                    {/* Actions */}
                    <div style={{display:"flex",gap:8}}>
                      ...
                      <button onClick={()=>setConfirmId(c.id)} style={{background:C.red+"22",border:"1px solid "+C.red+"44",color:C.red,borderRadius:7,padding:"6px 12px",fontSize:12,cursor:"pointer",fontWeight:700}}>🗑️ Supprimer</button>
                    </div>
                  </div>
                )}
              </div>
      ...
      <Card title="📊 Récapitulatif financier global">
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12,marginBottom:16}}>
          {[
            ...
            ["% consommé global",pct(chantiers.reduce((a,c)=>a+getBudgetConsomme(c),0),chantiers.reduce((a,c)=>a+c.budgetInitial,0))+"%",C.orange],
          ].map(([k,v,col])=>(
        ...
        {chantiers.map(c=>{
          ...
          return(
            <div key={c.id} style={{display:"flex",alignItems:"center",gap:12,padding:"8px 0",borderBottom:"1px solid "+C.border,fontSize:12}}>
              <div style={{minWidth:170,fontWeight:600,color:C.white}}>{c.nom.split(" ").slice(0,3).join(" ")}</div>
              <div style={{flex:1}}><PBar p={p} color={p>100?C.red:p>80?C.yellow:C.green} h={8}/></div>
              <div style={{minWidth:55,textAlign:"right",fontWeight:700,color:p>100?C.red:p>80?C.yellow:C.green}}>{p}%</div>
...
function AlertesPage({chantiers,openChantier}){
  const toutes=chantiers.flatMap(c=>c.alertes.map(a=>({alerte:a,chantier:c})));
  return(
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      {toutes.length===0&&<EmptyState msg="Aucune alerte 🎉" icon="✅"/>}
      {toutes.map(({alerte,chantier},i)=>(
        <div key={i} onClick={()=>openChantier(chantier.id)} style={{background:C.card,border:"1px solid "+C.red+"44",borderRadius:10,padding:"14px 18px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div><div style={{fontWeight:600,fontSize:14,color:C.red}}>⚠️ {alerte}</div><div style={{fontSize:12,color:C.muted,marginTop:4}}>{chantier.nom} · {chantier.client}</div></div>
...
// ═════════════════════════════════════════════════════════════════════════════
// KPI — basés sur dépenses réelles
// ═════════════════════════════════════════════════════════════════════════════
function KpiPage({chantiers}){
  ...
  const enBonne=chantiers.filter(c=>{const p=pct(getBudgetConsomme(c),c.budgetInitial);return p<80;});
  const clos=chantiers.filter(c=>c.statut==="Clôturé");
  const tauxDerive=Math.round(derives.length/total*100);
  const tauxSante=Math.round(enBonne.length/total*100);

  // Marge par chantier
  const marges=chantiers.map(c=>({name:c.nom.split(" ")[0],marge:Math.round((c.budgetInitial-getBudgetConsomme(c))/1000),pct:pct(getBudgetConsomme(c),c.budgetInitial)}));

  // Dépenses par catégorie globale
  const allDep=chantiers.flatMap(c=>c.depenses);
  const depCat=CATEGORIES.map(cat=>({cat,total:allDep.filter(d=>d.categorie===cat).reduce((a,d)=>a+d.montant,0)})).filter(x=>x.total>0);

  const radarData=[
    {kpi:"Budget OK",val:tauxSante},
  ...
  const bdData=chantiers.map(c=>({
    name:c.nom.split(" ").slice(0,2).join(" "),
    budget:Math.round(c.budgetInitial/1000),
    dep:Math.round(getBudgetConsomme(c)/1000),
    marge:Math.round((c.budgetInitial-getBudgetConsomme(c))/1000)
  }));
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:24}}>
      ...
      <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
        ...
        <KpiCard icon="🧾" label="Dépenses réelles" value={fmt(totalDeps)} color={C.yellow} sub={pctGlobal+"% du budget total"}/>
      ...
      <Card title="Consommation budgétaire globale">
        <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:6}}>
          <span style={{color:C.muted}}>Dépenses cumulées sur budget total</span>
        ...
        <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:C.muted,marginTop:6}}>
          <span>Dépensé : {fmt(totalDeps)}</span>
          <span>Restant : {fmt(margeGlobale)}</span>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
        ...
        <Card title="Répartition dépenses par catégorie (global)">
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={depCat} layout="vertical" margin={{left:10,right:10}}>
              ...
              <Bar dataKey="total" radius={[0,4,4,0]}>
                {depCat.map(({cat},i)=><Cell key={i} fill={catColor(cat)}/>)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </Card>
      ...
      {/* Budget vs Dépenses vs Marge */}
      <Card title="Budget vs Dépenses par chantier (k XOF)">
...
function IAPage({chantiers,openChantier,interventions}){
  ...
  const prest={};
  allDep.filter(d=>d.categorie==="Sous-traitance").forEach(d=>{const k=d.libelle.trim();if(!prest[k])prest[k]={libelle:k,total:0,chantiers:new Set()};prest[k].total+=d.montant;prest[k].chantiers.add(d.chantier);});
  const prestList=Object.values(prest).sort((a,b)=>b.total-a.total);
  const totalST=allDep.filter(d=>d.categorie==="Sous-traitance").reduce((a,d)=>a+d.montant,0);

  const runIA=async()=>{
    setAnalysing(true);setIaError(null);setIaResult(null);
    try{
      const ctx={
        chantiers:chantiers.map(c=>({nom:c.nom,client:c.client,statut:c.statut,budgetInitial:c.budgetInitial,depensesTotal:totalDep(c),score:c.score,depenses:c.depenses})),
        ...
        doublons:doublons.map(([lib,items])=>({libelle:lib,occurrences:items.length,total:items.reduce((a,i)=>a+i.montant,0),chantiers:items.map(i=>i.chantier)})),
        prestataires:prestList.map(p=>({libelle:p.libelle,total:p.total,chantiers:[...p.chantiers]}))
      };
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:"Expert BTP. Analyse ce portefeuille (XOF). Réponds UNIQUEMENT en JSON:\n"+JSON.stringify(ctx)+"\n\nFormat: {\"recapPrestataires\":[{\"nom\":string,\"totalXOF\":number,\"risque\":\"faible\"|\"moyen\"|\"élevé\",\"commentaire\":string}],\"alertesDoublons\":[{\"libelle\":string,\"occurrences\":number,\"totalXOF\":number,\"action\":string}],\"depensesSuspectes\":[{\"libelle\":string,\"chantier\":string,\"montant\":number,\"raison\":string}],\"recommandations\":[{\"titre\":string,\"detail\":string,\"priorite\":\"haute\"|\"moyenne\"|\"basse\"}],\"scoreGlobal\":number,\"synthese\":string}"}]})});
      const data=await res.json();
      const text=(data.content||[]).map(i=>i.text||"").join("");
      setIaResult(JSON.parse(text.replace(/```json|```/g,"").trim()));
    }catch(e){setIaError("Erreur analyse IA.");}
    setAnalysing(false);
  };
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      {activeTab==="derives"&&(
        <>
          <Card title="🚨 Dérives budgétaires (basées sur les dépenses)">
          ...
          {doublons.length===0?<div style={{color:C.green,padding:16,textAlign:"center"}}>✅ Aucun doublon</div>:<>
            ...
            {doublons.map(([lib,items])=>(
              <div key={lib} style={{border:"1px solid "+C.yellow+"44",borderRadius:10,padding:"14px 16px",marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><div style={{fontWeight:700,color:C.yellow}}>🔁 "{items[0].libelle}"</div><span style={{fontSize:12,color:C.muted}}>{items.length}x · {fmt(items.reduce((a,i)=>a+i.montant,0))}</span></div>
                {items.map((item,idx)=>(
      ...
      {activeTab==="ia"&&(
        <>
          ...
          {analysing&&<EmptyState msg="Analyse en cours..." icon="⏳"/>}
          {iaResult&&(
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              ...
              {iaResult.recapPrestataires && iaResult.recapPrestataires.length>0 && (
                <Card title="👷 Prestataires">
                  {iaResult.recapPrestataires.map((p,i)=>(
                    <div key={i} style={{padding:"12px 0",borderBottom:"1px solid "+C.border,display:"flex",justifyContent:"space-between",gap:10}}>
                      ...
                      <div style={{display:"flex",gap:8,alignItems:"center"}}>
                        <span style={{fontWeight:700,color:C.orange}}>{fmt(p.totalXOF)}</span>
                        <Badge label={"Risque "+p.risque} color={p.risque==="élevé"?C.red:p.risque==="moyen"?C.yellow:C.green} small/>
                      </div>
                    </div>
                  ))}
                </Card>
